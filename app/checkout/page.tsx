...
function CheckoutContent() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        setCar(carData);
        setPkg(packagesData.find((p: Package) => p.id === pkgId) || null);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const handleConfirm = async () => {
    if (!user || !car || !pkg) {
      alert('Missing required transaction data.');
      return;
    }
                ...
                >
                  ...
                  <div className="mt-12 p-8 border border-accent/20 bg-surface-accent">
                    ...
                    <p className="mono-text text-[9px] text-muted leading-relaxed">
                      BOOKING AS IDENTITY::[ {(user?.name || 'UNKNOWN').toUpperCase()} ]. OPERATOR MUST PRESENT VALID BIOMETRIC ID AND CREDIT INSTRUMENTS AT DEPLOYMENT NODE.
                    </p>
                  </div>
        ...
        <aside className={styles.summarySidebar}>
          <div className={styles.summaryCard}>
            ...
            <div className="mb-10 text-center">
              ...
              <p className="mono-text text-[9px] text-muted mt-2">{car?.year} {"//"} {(pkg?.name || 'STANDARD').toUpperCase()}_CLASS</p>
            </div>
